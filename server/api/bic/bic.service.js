...
var BICS_URL = 'https://api.businessdescription.co.nz/api/bics';
...
function index() {
  return _Promise.resolve().then(function () {
    ...
    return jsonfile.readFileAsync(BICS_SEED_FILENAME).then(function (bics) {
        ...
        return _Promise.all((0, _lodash2['default'])(industries).map(function (industry) {
          ...
          return _Promise.all((0, _lodash2['default'])(divisions).flatten().map(function (division) {
            ...
            return bicsRequest.then(function (bics) {
              return (0, _lodash2['default'])(classes).flatten().map(function (clazz) {
                clazz.className = clazz.name;
                clazz.classId = clazz.id;
                return clazz;
              }).map(function (clazz) {
                var bic = _lodash2['default'].find(bics, _lodash2['default'].matchesProperty('classId', clazz.classId));
                return _lodash2['default'].merge({}, clazz, bic);
              }).map(function (clazz) {
